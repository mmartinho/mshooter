...
class Compra {
    ...
    static async busca(esportista_id, pce_id, documento_id) {
        const pceDoEsportista = await Pce.itemDoEsportista(pce_id, esportista_id);
        if(!pceDoEsportista) {
     ...
     */
     static async lista(esportista_id, pce_id) {
        ...
        const all = await db.Compra.findAll({ where: { pce_id }, include: 'Documento' });
        return all;    
    }    
    ...
    static async existe(esportista_id, pce_id, documento_id) {
        ...
        const existe = await Compra.busca(pce_id, documento_id);
        if(existe) {
            return true;
        } else {
            return false;
